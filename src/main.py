...
class Player:
    ...
    def update(self):
        ...
        if (
            ...
            and self.start_y - JUMP_HEIGHT < self.y
            and self.falling == False
        ):
            self.d_y = -6  # if space pressed begins to add -1 to self.y

        self.x, self.y, self.d_x, self.d_y, collision_side = move(
            self.x, self.y, self.d_x, self.d_y
        )
...
class Sword:
    ...
    def draw(self):
        ...
        if self.active:
            pyxel.blt(
                self.x, self.y, img=0, u=u, v=v, w=w, h=8, colkey=TRANSPARENT_COLOR
            )
...
class App:
    def __init__(self):
        pyxel.init(128, 128, title="Pyxel Platformer", fps=35)
